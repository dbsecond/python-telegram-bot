...
class StyleTransferModel:
    ...
    def _get_style_model_and_losses(self, cnn, normalization_mean, normalization_std,
    ...
    def process_image(self, content_img, style_img):

        content_image = self._image_loader(content_img)
        input_image = content_image.clone()
        style_image = self._image_loader(style_img)

        output_image = self._run_style_transfer(self.cnn, self.cnn_normalization_mean, self.cnn_normalization_std, content_image, style_image, input_image)

        return unloader(output_image.cpu().clone().squeeze(0))
